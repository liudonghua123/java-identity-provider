...
if ERRORLEVEL 1 (
  cd ..
  echo As exptected embedded/jetty-base-9.3 directory not found.
  echo Place jetty base for 9.3 into idp-extract\%idpex%\jetty-base and run the -post script
  goto done;
)
echo "Do not run this script against a post 3.2.1 build
goto done;
